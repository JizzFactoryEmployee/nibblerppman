...
def DASH():
    print('DASHMAGIC START')
    try:
        data = pd.read_csv(r'/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/DASH/1m/DASH1m.csv')
    ...
    my_cursor.execute(''' select count(*) from DASH; ''')
    ...
    try:
        ...
        pass
    print('DASH the database length is equal to :',result)
    print('DASH the csv length is equal to:', len(data))


    if result == 0 or result == None:
        
        print('====DASH DATABASE IS EMPTY TIME TO POPULATE=======')


        my_cursor = my_conn.cursor()
        start1 = time.time()
        #pushing data into the database from the CSV file
        my_cursor.execute(''' LOAD DATA LOCAL INFILE '/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/DASH/1m/DASH1m.csv' IGNORE INTO TABLE DASH
    ...
    if result < len(data) and result > 0:
        ...
        for i in to_push:
            ...
            start2 = time.time()
            my_cursor = my_conn.cursor()
            my_cursor.execute('INSERT INTO DASH VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (pair_1, pair_2, Date_Time, Open_price, High_price, Low_price, Close_price, Volume))
            my_conn.commit()
            end2 = time.time()


    if result > len(data):
        print('somDASHing went wrong, database is somehow longer than the csv, deleting all')
        my_cursor = my_conn.cursor()
