...
def IOST():
    print('IOSTMAGIC START')
    try:
        data = pd.read_csv(r'/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/IOST/1m/IOST1m.csv')
    ...
    my_cursor.execute(''' select count(*) from IOST; ''')
    ...
    try:
        ...
        pass
    print('IOST the database length is equal to :',result)
    print('IOST the csv length is equal to:', len(data))


    if result == 0 or result == None:
        
        print('====IOST DATABASE IS EMPTY TIME TO POPULATE=======')


        my_cursor = my_conn.cursor()
        start1 = time.time()
        #pushing data into the database from the CSV file
        my_cursor.execute(''' LOAD DATA LOCAL INFILE '/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/IOST/1m/IOST1m.csv' IGNORE INTO TABLE IOST
    ...
    if result < len(data) and result > 0:
        ...
        for i in to_push:
            ...
            Open_price = str(round(to_push[y][3], 7))
            High_price = str(round(to_push[y][4], 7))
            Low_price = str(round(to_push[y][5], 7))
            Close_price = str(round(to_push[y][6], 7))
            Volume = str(round(to_push[y][7], 7))
            y = y+1
            start2 = time.time()
            my_cursor = my_conn.cursor()
            my_cursor.execute('INSERT INTO IOST VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (pair_1, pair_2, Date_Time, Open_price, High_price, Low_price, Close_price, Volume))
            my_conn.commit()
            end2 = time.time()

    if result > len(data):
        print('somIOSTing went wrong, database is somehow longer than the csv, deleting all')
        my_cursor = my_conn.cursor()
