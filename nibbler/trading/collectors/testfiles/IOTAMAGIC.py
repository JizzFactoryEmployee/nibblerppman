...
def IOTA():
    print('IOTAMAGIC START')
    try:
        data = pd.read_csv(r'/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/IOTA/1m/IOTA1m.csv')
    ...
    my_cursor.execute(''' select count(*) from IOTA; ''')
    ...
    try:
        ...
        pass
    print('IOTA the database length is equal to :',result)
    print('IOTA the csv length is equal to:', len(data))


    if result == 0 or result == None:
        
        print('====IOTA DATABASE IS EMPTY TIME TO POPULATE=======')


        my_cursor = my_conn.cursor()
        start1 = time.time()
        #pushing data into the database from the CSV file
        my_cursor.execute(''' LOAD DATA LOCAL INFILE '/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/IOTA/1m/IOTA1m.csv' IGNORE INTO TABLE IOTA
    ...
    if result < len(data) and result > 0:
        ...
        for i in to_push:
            ...
            Open_price = str(round(to_push[y][3], 5))
            High_price = str(round(to_push[y][4], 5))
            Low_price = str(round(to_push[y][5], 5))
            Close_price = str(round(to_push[y][6], 5))
            Volume = str(round(to_push[y][7], 5))
            y = y+1
            start2 = time.time()
            my_cursor = my_conn.cursor()
            my_cursor.execute('INSERT INTO IOTA VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (pair_1, pair_2, Date_Time, Open_price, High_price, Low_price, Close_price, Volume))
            my_conn.commit()
            end2 = time.time()

    if result > len(data):
        print('somIOTAing went wrong, database is somehow longer than the csv, deleting all')
        my_cursor = my_conn.cursor()
