...
def ONT():
    print('ONTMAGIC START')
    try:
        data = pd.read_csv(r'/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/ONT/1m/ONT1m.csv')
    ...
    my_cursor.execute(''' select count(*) from ONT; ''')
    ...
    try:
        ...
        pass
    print('ONT the database length is equal to :',result)
    print('ONT the csv length is equal to:', len(data))


    if result == 0 or result == None:
        
        print('====ONT DATABASE IS EMPTY TIME TO POPULATE=======')


        my_cursor = my_conn.cursor()
        start1 = time.time()
        #pushing data into the database from the CSV file
        my_cursor.execute(''' LOAD DATA LOCAL INFILE '/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/ONT/1m/ONT1m.csv' IGNORE INTO TABLE ONT
    ...
    if result < len(data) and result > 0:
        ...
        for i in to_push:
            ...
            start2 = time.time()
            my_cursor = my_conn.cursor()
            my_cursor.execute('INSERT INTO ONT VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (pair_1, pair_2, Date_Time, Open_price, High_price, Low_price, Close_price, Volume))
            my_conn.commit()
            end2 = time.time()

    if result > len(data):
        print('somONTing went wrong, database is somehow longer than the csv, deleting all')
        my_cursor = my_conn.cursor()
