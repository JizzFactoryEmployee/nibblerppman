...
def XLM():
    print('XLMMAGIC START')
    try:
        data = pd.read_csv(r'/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/XLM/1m/XLM1m.csv')
    ...
    my_cursor.execute(''' select count(*) from XLM; ''')
    ...
    try:
        ...
        pass
    print('XLM the database length is equal to :',result)
    print('XLM the csv length is equal to:', len(data))


    if result == 0 or result == None:
        
        print('====XLM DATABASE IS EMPTY TIME TO POPULATE=======')


        my_cursor = my_conn.cursor()
        start1 = time.time()
        #pushing data into the database from the CSV file
        my_cursor.execute(''' LOAD DATA LOCAL INFILE '/home/nibbler/nibblerppman/nibbler/trading/collectors/coins/XLM/1m/XLM1m.csv' IGNORE INTO TABLE XLM
    ...
    if result < len(data) and result > 0:
        ...
        for i in to_push:
            ...
            Open_price = str(round(to_push[y][3], 6))
            High_price = str(round(to_push[y][4], 6))
            Low_price = str(round(to_push[y][5], 6))
            Close_price = str(round(to_push[y][6], 6))
            Volume = str(round(to_push[y][7], 6))
            y = y+1
            start2 = time.time()
            my_cursor = my_conn.cursor()
            my_cursor.execute('INSERT INTO XLM VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (pair_1, pair_2, Date_Time, Open_price, High_price, Low_price, Close_price, Volume))
            my_conn.commit()
            end2 = time.time()

    if result > len(data):
        print('somXLMing went wrong, database is somehow longer than the csv, deleting all')
        my_cursor = my_conn.cursor()
